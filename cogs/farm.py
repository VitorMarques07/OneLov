...
class RejectModal(discord.ui.Modal, title='❌ Motivo da reprovação'):
 ...
 async def on_submit(self,i):
  cog=self.bot.get_cog('Farm'); await cog.finish_review(i,self.did,'rejected',str(self.motivo.value))

class Review(discord.ui.View):
 def __init__(self,bot,did): super().__init__(timeout=86400); self.bot=bot; self.did=did
...
class DeliveryModal(discord.ui.Modal, title='📦 Registrar Entrega'):
 ...
 async def on_submit(self,i):
  ...
  if attachment.content_type and not attachment.content_type.startswith('image/'):
   return await i.response.send_message('❌ Envie uma imagem (JPG, PNG, WEBP etc.) como comprovante.',ephemeral=True)
...
class Farm(commands.Cog):
 ...
 async def register_delivery(self,i,quantidade,observacao,attachment):
  ...
  did=await self.bot.db.execute('INSERT INTO deliveries(guild_id,week_id,ticket_id,member_id,quantity,status,registered_by,created_at,attachment_url,attachment_name) VALUES(?,?,?,?,?,?,?,?,?,?)',(i.guild_id,week['id'],None,i.user.id,quantidade,status,i.user.id,now.isoformat(),attachment.url,attachment.filename))
  await self.bot.db.log(i.guild_id,i.user.id,'entrega_registrada',f'id={did};qtd={quantidade};arquivo={attachment.filename};obs={observacao}')
  if status=='approved':
   await i.response.send_message(f'🟢 **Entrega #{did} aprovada automaticamente.**\n📦 Farm Completo: **{quantidade:,}**'.replace(',','.'),ephemeral=True); return
  log_channel_id=cfg['log_channel_id'] if cfg else 0
  channel=i.guild.get_channel(log_channel_id) if log_channel_id else None
  ...
  embed=discord.Embed(title=f'📋 ENTREGA #{did}',description=f'👤 Membro: {i.user.mention}\n📦 Material: **Farm Completo**\n🔢 Quantidade: **{quantidade:,}**\n🟡 Status: **Aguardando aprovação**\n📝 Observação: {observacao or "-"}'.replace(',','.'),color=discord.Color.orange())
  embed.set_image(url=attachment.url); embed.set_footer(text=f'Comprovante: {attachment.filename}')
  msg=await channel.send(embed=embed,view=Review(self.bot,did)); await self.bot.db.execute('UPDATE deliveries SET review_channel_id=?,review_message_id=? WHERE id=?',(channel.id,msg.id,did))
  await i.response.send_message(f'✅ **Entrega #{did} enviada para análise.**\n🟡 Aguarde a aprovação da equipe.',ephemeral=True)
 ...
 async def delivery(self,i,quantidade:int):
  await i.response.send_message('📦 Use o botão **FAZER ENTREGA** em #📦・entregas-de-farm para anexar o comprovante.',ephemeral=True)
