...
class Admin(commands.Cog):
 ...
 @app_commands.command(name='config',description='Configura o sistema FARM MANAGER.')
 ...
 async def startweek(self,i):
  ...
  await self.bot.ensure_week(i.guild); await i.response.send_message('✅ Semana atual inicializada.')
 @app_commands.command(name='cobrar',description='Executa a cobrança manual nos tickets ativos.')
 async def charge(self,i):
  ...
  await self.bot.ensure_week(i.guild); await self.bot.send_charges(i.guild,self.bot.local_now(),True); await self.bot.db.log(i.guild_id,i.user.id,'cobranca_manual'); await i.response.send_message('🔔 Cobrança manual enviada nos tickets ativos.')
