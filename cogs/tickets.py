...
class TicketView(discord.ui.View):
 ...
 @discord.ui.button(label='ABRIR TICKET',style=discord.ButtonStyle.green,custom_id='onelov:ticket')
 async def open(self,i,b):
  ...
  c=await i.guild.create_text_channel(f'farm-{i.user.name}'[:90],category=cat,overwrites=ow); tid=await self.bot.db.execute('INSERT INTO tickets(guild_id,channel_id,member_id,opened_at) VALUES(?,?,?,?)',(i.guild_id,c.id,i.user.id,datetime.utcnow().isoformat())); await self.bot.db.log(i.guild_id,i.user.id,'ticket_criado',f'ticket={tid}'); await c.send(f'🎫 **OneLov — Ticket de Farm**\n👤 Membro: {i.user.mention}\n\n📦 Registre sua entrega pelo botão abaixo.',view=DeliveryView(self.bot)); await i.response.send_message(f'✅ {c.mention}',ephemeral=True)
...
class Tickets(commands.Cog):
 ...
 async def panel(self,i):
  ...
  await i.response.send_message('🎫 **OneLov — PAINEL DE TICKETS**\nClique para abrir seu ticket.',view=TicketView(self.bot))
