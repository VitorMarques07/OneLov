...
class Setup(commands.Cog):
 ...
 async def permissions(self,guild,category,staff,admin,channels):
  everyone=guild.default_role; bot=guild.me; base={everyone:discord.PermissionOverwrite(view_channel=True,send_messages=False,read_message_history=True),staff:discord.PermissionOverwrite(view_channel=True,send_messages=False,read_message_history=True),admin:discord.PermissionOverwrite(view_channel=True,send_messages=False,manage_messages=True,manage_channels=True,read_message_history=True)}
  ...
  for key,ch in channels.items():
   ow=dict(base)
   if key in ('entregas','logs'): ow[everyone]=discord.PermissionOverwrite(view_channel=False)
   await ch.edit(overwrites=ow,reason='OneLov /setup')
 async def grant_defaults(self,guild,staff,admin):
  for role in (staff,admin):
   for p in ('aprovação','registro','membros','meta','dashboard','relatórios','cobranças','logs','tickets'): await self.bot.db.execute('INSERT OR IGNORE INTO permissions(guild_id,permission,role_id,user_id) VALUES(?,?,?,NULL)',(guild.id,p,role.id))
  if settings.super_admin_id:
   for p in ('configuração','aprovação','registro','membros','meta','dashboard','relatórios','cobranças','logs','tickets'): await self.bot.db.execute('INSERT OR IGNORE INTO permissions(guild_id,permission,role_id,user_id) VALUES(?,?,NULL,?)',(guild.id,p,settings.super_admin_id))
 async def panel(self,ch,key):
  marker=f'onelov-panel:{key}'
  async for m in ch.history(limit=30):
   footer=(m.embeds[0].footer.text if m.embeds and m.embeds[0].footer else '')
   if m.author==self.bot.user and marker in footer: return
 ...
 async def setup_command(self,interaction:discord.Interaction):
  ...
  await interaction.response.defer(ephemeral=True,thinking=True); guild=interaction.guild; category=discord.utils.get(guild.categories,name='ONELOV') or await guild.create_category('ONELOV',reason='OneLov /setup')
  staff=await self.role(guild,'OneLov Staff'); admin=await self.role(guild,'OneLov Admin'); channels={k:await self.channel(guild,n,category) for k,n in CHANNELS.items()}; await self.permissions(guild,category,staff,admin,channels); await self.grant_defaults(guild,staff,admin)
  for key,ch in channels.items():
   if key!='logs': await self.panel(ch,key)
  await self.bot.db.execute('INSERT INTO guild_config(guild_id,weekly_goal,approval_required,ticket_category_id,log_channel_id,cobranca_hour,cobranca_minute,auto_charge) VALUES(?,?,?,?,?,?,?,?) ON CONFLICT(guild_id) DO UPDATE SET weekly_goal=2000,approval_required=1,ticket_category_id=excluded.ticket_category_id,log_channel_id=excluded.log_channel_id',(guild.id,2000,1,category.id,channels['logs'].id,18,0,1)); await self.bot.ensure_week(guild)
