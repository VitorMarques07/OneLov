...
class FarmManager(commands.Bot):
 def __init__(self):
  super().__init__(command_prefix='!',intents=intents,application_id=settings.client_id or None)
  self.db=Database(settings.database_path); self.tz=ZoneInfo(settings.timezone)
 ...
 async def weekly_automation(self):
  ...
  if now.weekday()!=0 or now.hour!=0 or now.minute!=0: return
  for guild in self.guilds:
   ...
   start=now.replace(hour=0,minute=0,second=0,microsecond=0); end=start+timedelta(days=6,hours=23,minutes=59,seconds=59)
   ...
   if cfg and cfg['auto_charge']: await self.send_charges(guild,now)
 async def send_charges(self,guild,now,manual=False):
  cfg=await self.db.one('SELECT * FROM guild_config WHERE guild_id=?',(guild.id,))
  if not cfg:return
  start=now-timedelta(days=now.weekday()); week=await self.db.one('SELECT id FROM weeks WHERE guild_id=? AND start_date LIKE ?',(guild.id,start.strftime('%Y-%m-%d')+'%',))
  if not week:return
