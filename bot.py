...
class OneLov(commands.Bot):
    ...
    async def send_charges(self, guild, now, manual=False):
        cfg = await self.db.one('SELECT * FROM guild_config WHERE guild_id=?', (guild.id,))
        week = await self.ensure_week(guild, now)
        if not cfg:
            return
        for ticket in await self.db.all("SELECT * FROM tickets WHERE guild_id=? AND status='open'", (guild.id,)):
            member = guild.get_member(ticket['member_id'])
            channel = guild.get_channel(ticket['channel_id'])
            if not member or not channel:
                continue
            if not manual and await self.db.one('SELECT id FROM charges WHERE week_id=? AND channel_id=?', (week['id'], channel.id)):
                continue
